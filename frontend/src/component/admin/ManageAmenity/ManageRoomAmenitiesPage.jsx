...
const ManageRoomAmenitiesPage = () => {
	...
	const fetch = async () => {
		...
		try {
			...
			setError(err.message || 'Error fetching room amenities');
			setItems([]);
		}
		setLoading(false);
	};
	...
	const openEdit = () => {
		...
		setEditing(id); setForm({ name: a.name || '', description: a.description || '' }); setShowModal(true);
	};
	...
	const submit = async e => {
		...
		try {
			if (editing) {
				await updateRoomAmenity(editing, form);
				setMessage('Tiện nghi đã cập nhật');
			} else {
				await addRoomAmenity(form);
	...
	return (
		<div className="container mx-auto p-6 mt-20">
			<h1 className="text-2xl font-bold mb-2">Quản lý tiện nghi phòng</h1>
			<p className="text-gray-600 mb-4">Tạo, chỉnh sửa hoặc xóa các tiện nghi có thể gắn vào phòng.</p>
			...
			<div className="bg-white rounded shadow overflow-hidden">
				{loading ? (
					...
					<div className="p-4 text-gray-600">Không tìm thấy tiện nghi phòng nào.</div>
				) : (
...
export default ManageRoomAmenitiesPage;
